...
public class ExToast {
    ...
    public void show() {
        ...
        initTN();
        try {
            show.invoke(mTN,mSetView.getWindowToken());
        } catch (Exception e) {
            e.printStackTrace();
        }
        isShow = true;
    ...
    public static ExToast makeToast(Context context,View rootView,int duration){
        Toast toast = new Toast(context);
        ExToast exToast = new ExToast(context);
        mSetView = rootView;
        toast.setView(mSetView);
        if (mSetView instanceof DanmuLayout){
            ((DanmuLayout) mSetView).setDanmuContent("测试弹幕");
        }
        exToast.toast = toast;
    ...
    private void initTN() {
        try {
            ...
            if (null != mTN){
                show = mTN.getClass().getDeclaredMethod("show",new Class[]{IBinder.class});
